...
Suppose the true supply function for chicken is given by price, and one (or more) exogenous supply shifers $$Z_{supply}$$: 
...
$$\textcolor{red}{NOTE}$$: In equilibrium, price and quantity only depend on the exogenous supply and demand shifters. I think this one intuition for why we include all exogenous variables in the first stage.  
...
# price using reduced form equation
p   <- 50 + z1 + z2 + v 
...
if (sum(qd - qs) > 1e10) {
    cat("ERROR: Supply and demand not balanced")
}
...
#### 2SLS - Only Supply Shifters in First Stage
...
#### 2SLS - All Exogenous Variables in First Stage
